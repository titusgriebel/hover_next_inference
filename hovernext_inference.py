...
def postprocess_inference(path):
    for image_dir in natsorted(glob(os.path.join(path, '*'))):
        ...
        pred = zarr.open(os.path.join(image_dir, 'pinst_pp'), mode='r')
        inference = np.squeeze(pred)
        imageio.imwrite(os.path.join(path, f'{os.path.basename(image_dir)}.tiff'), inference, format="TIFF")
...
run_inference(input_dir='/mnt/lustre-grete/usr/u12649/scratch/data/test', 
